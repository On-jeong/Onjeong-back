...
public class MailController {
    ...
    @PostMapping("/mails/send/{mailId}")
    public ResponseEntity<Boolean> deleteSendMail(@PathVariable("mailId") Long mailId) {
        return ResponseEntity.ok(mailService.deleteSendMail(mailId));
    }

    @ApiOperation(value = "받은 메시지 삭제")
    @PostMapping("/mails/receive/{mailId}")
    public ResponseEntity<Boolean> deleteReceiveMail(@PathVariable("mailId") Long mailId) {
